...
void C输入::f显示() {
	...
	f画方向({-c上x, c上y}, m手柄->f方向((输入x::t索引)输入x::E手柄方向::e左摇杆));
	f画方向({c下x, c下y}, m手柄->f方向((输入x::t索引)输入x::E手柄方向::e右摇杆));
	f画四键({-c下x, c下y}, m手柄->f按键((输入x::t索引)输入x::E手柄按键::e左), m手柄->f按键((输入x::t索引)输入x::E手柄按键::e上), m手柄->f按键((输入x::t索引)输入x::E手柄按键::e右), m手柄->f按键((输入x::t索引)输入x::E手柄按键::e下));
	f画方向({-c下x, c下y}, m手柄->f方向((输入x::t索引)输入x::E手柄方向::e方向键));
	f画四键({c上x, c上y}, m手柄->f按键((输入x::t索引)输入x::E手柄按键::x), m手柄->f按键((输入x::t索引)输入x::E手柄按键::y), m手柄->f按键((输入x::t索引)输入x::E手柄按键::b), m手柄->f按键((输入x::t索引)输入x::E手柄按键::a));
	f画按键({-c上x, c肩y}, m手柄->f按键((输入x::t索引)输入x::E手柄按键::e左肩));
	f画按键({c上x, c肩y}, m手柄->f按键((输入x::t索引)输入x::E手柄按键::e右肩));
	f画触发({-c上x, c肩y}, m手柄->f触发((输入x::t索引)输入x::E手柄触发::e左));
	f画触发({c上x, c肩y}, m手柄->f触发((输入x::t索引)输入x::E手柄触发::e右));
	//结束
	v渲染控制.f结束();
}
